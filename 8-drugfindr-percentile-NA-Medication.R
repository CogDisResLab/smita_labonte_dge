...
dge <- read.csv("Galaxy Output/NA/NA_Male_Medication_DGE.csv") %>% mutate(GeneID = as.character(GeneID))
...
drugfind_results <- investigate_signature(dge, "CP",
  ...
  write_csv("figures/DrugFindR_Output/NA/NA_Medication1.csv")
...
drugfind_results_2 <- investigate_signature(dge, "CP",
                                            ...
                                            pval_column = "PValue",
                                            source_name = "NA_Medication") |>
  write_csv("figures/DrugFindR_Output/NA/NA_Medication2.csv")
