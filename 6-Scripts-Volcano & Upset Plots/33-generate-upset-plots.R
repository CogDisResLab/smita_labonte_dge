...
files <- list.files(file.path("results/2. DGEs (Galaxy) Post SVA"), "csv", recursive = TRUE)

filenames <- str_remove(files, "_DGE.csv") %>%
  str_remove("\\w+/")

filepaths <- file.path("results/2. DGEs (Galaxy) Post SVA", files)
...
Anterior_Insula <-
  list(
    Anterior_Insula_female = results$AI_Females,
    Anterior_Insula_male = results$AI_Males,
    Anterior_Insula_overall = results$AI_Overall,
    Anterior_Insula_medication = results$AI_MalesMeds
  )

Cingulate_Gyrus <-
  list(
    Cingulate_Gyrus_female = results$CG_Females,
    Cingulate_Gyrus_male = results$CG_Males,
    Cingulate_Gyrus_overall = results$CG_Overall,
    Cingulate_Gyrus_medication = results$CG_MalesMeds
  )

DLPFC <-
  list(DLPFC_female = results$DLPFC_Females,
       DLPFC_male = results$DLPFC_Males,
       DLPFC_overall = results$DLPFC_Overall,
       DLPFC_medication = results$DLPFC_MalesMeds
       )
...
Nucleus_Accumbens <-
  list(
    Nucleus_Accumbens_female = results$NA_Females,
    Nucleus_Accumbens_male = results$NA_Males,
    Nucleus_Accumbens_overall = results$NA_Overall,
    Nucleus_Accumbens_medication = results$NA_MalesMeds
  )

Orbitofrontal_Cortex <-
  list(
    Orbitofrontal_Cortex_female = results$OFC_Females,
    Orbitofrontal_Cortex_male = results$OFC_Males,
    Orbitofrontal_Cortex_overall = results$OFC_Overall,
    Orbitofrontal_Cortex_medication = results$OFC_MalesMeds
  )

Subbiculum <-
  list(
    Subbiculum_female = results$SUB_Females,
    Subbiculum_male = results$SUB_Males,
    Subbiculum_overall = results$SUB_Overall,
    Subbiculum_medication = results$SUB_MalesMeds
  )
...
png(filename = "results/8. Upset Plots/Anterior_Insula_Upset.png", width = 10, height = 8, units = "in", res = 300)
upset_plot_A
dev.off()

png(filename = "results/8. Upset Plots/Cingulate_Gyrus_Upset.png", width = 10, height = 8, units = "in", res = 300)
upset_plot_C
dev.off()

png(filename = "results/8. Upset Plots/DLPFC_Upset.png", width = 10, height = 8, units = "in", res = 300)
upset_plot_D
dev.off()
...
png(filename = "results/8. Upset Plots/Nucleus_Accumbens_Upset.png", width = 10, height = 8, units = "in", res = 300)
upset_plot_N
dev.off()

png(filename = "results/8. Upset Plots/Orbitofrontal_Cortex_Upset.png", width = 10, height = 8, units = "in", res = 300)
upset_plot_O
dev.off()

png(filename = "results/8. Upset Plots/Subbiculum_Upset.png", width = 10, height = 8, units = "in", res = 300)
upset_plot_S
dev.off()
