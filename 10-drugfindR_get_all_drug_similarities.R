...
#Input DGE matrix
dge <- read_csv("results/3. L1000 Heat Maps Post SVA/Human_DGE-L1000_Matrix.csv")
...
ai_up_sig <- ai_dge_sig |>
  ...
  write_csv("results/8. DrugFindR Output/AI/L1000/Overall/AI_top_genes_sig.csv")

#Extract bottom X% gene signature
ai_dn_sig <- ai_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/8. DrugFindR Output/AI/L1000/Overall/AI_bottom_genes_sig.csv")
...
x <- ai_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/8. DrugFindR Output/AI/L1000/Overall/AI_Overall_L1000_drugfindr.xlsx")
