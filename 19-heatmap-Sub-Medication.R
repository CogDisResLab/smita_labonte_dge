...
dge <- read.csv("Galaxy Output/Sub/Sub_Medication_DGE.csv") %>% mutate(GeneID = as.character(GeneID))
...
count <- read.csv("Galaxy Count Matrix Output/Sub/Sub_Male_Med_Counts.csv") %>% mutate(Geneid = as.character(Geneid))
...
colnames(lctmtx) <- paste0("Sample", 1:8)
