...
dge <- read.csv("Galaxy Output/Sub/Sub_Male_DGE.csv") %>% mutate(GeneID = as.character(GeneID))
...
count <- read.csv("Galaxy Count Matrix Output/Sub/Sub_Male_Counts.csv") %>% mutate(Geneid = as.character(Geneid))
...
colnames(lctmtx) <- paste0("Sample", 1:24)
