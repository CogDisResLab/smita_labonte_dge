# Extract top bottom 25% of genes from L1000 for each brain region and generate heat map (Mouse Females Comparison)
...
dge_na_symboled <- dge_na |>
  ...
  drugfindR::prepareSignature(geneColumn = "SYMBOL", logfcColumn = "NAC") |>
...
diffexp <- replace(diffexp, is.na(diffexp), 0) |>
  ...
  write_csv("results/4. Mouse Top 25% Heat Maps No SVA/Females/Mouse_DGE_L100025%_Females_Matrix.csv")

# Heatmap Generation ------------------------------------------------------

diff_expr_modified <- read_csv("results/4. Mouse Top 25% Heat Maps No SVA/Females/Mouse_DGE_L100025%_Females_Matrix.csv") |>
