# Extract top bottom 50% of genes from L1000 for each brain region and generate heat map (Overall Comparison)
...
#Obtain the top 50% DEGs from our dataframe
dge_ai_top <- dge_ai |>
  slice_max(AI, prop = 0.5)

#Obtain the bottom 50% DEGs from our dataframe
dge_ai_bottom <- dge_ai |>
  slice_min(AI, prop = 0.5)
...
dge_cg_top <- dge_cg |>
  slice_max(CG, prop = 0.5)

dge_cg_bottom <- dge_cg |>
  slice_min(CG, prop = 0.5)
...
dge_dlpfc_top <- dge_dlpfc |>
  slice_max(DLPFC, prop = 0.5)

dge_dlpfc_bottom <- dge_dlpfc |>
  slice_min(DLPFC, prop = 0.5)
...
dge_na_top <- dge_na |>
  slice_max(NAC, prop = 0.5)

dge_na_bottom <- dge_na |>
  slice_min(NAC, prop = 0.5)
...
dge_ofc_top <- dge_ofc |>
  slice_max(OFC, prop = 0.5)

dge_ofc_bottom <- dge_ofc |>
  slice_min(OFC, prop = 0.5)
...
dge_sub_top <- dge_sub |>
  slice_max(SUB, prop = 0.5)

dge_sub_bottom <- dge_sub |>
  slice_min(SUB, prop = 0.5)
...
diffexp <- replace(diffexp, is.na(diffexp), 0) |>
  write_csv("results/5. Top 50% Heat Maps Post SVA/Overall/Human_DGE_L1000_50%_Matrix.csv")

# Heatmap Generation ------------------------------------------------------

diff_expr_modified <- read_csv("results/5. Top 50% Heat Maps Post SVA/Overall/Human_DGE_L1000_50%_Matrix.csv") |>
