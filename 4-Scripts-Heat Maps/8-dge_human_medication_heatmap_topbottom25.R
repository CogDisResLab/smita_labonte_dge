# Extract top bottom 25% of genes from L1000 for each brain region and generate heat map (Males Meds Comparison)
...
dge_ai <- read_csv(files[1], col_types = cols(.default = col_character())) |>
  ...
  drugfindR::prepareSignature(geneColumn = "GeneID") |>
  dplyr::select(Name_GeneSymbol, AI = Value_LogDiffExp, AI_Pval = Significance_pvalue )

#Obtain the top 25% DEGs from our dataframe
dge_ai_top <- dge_ai |>
  slice_max(AI, prop = 0.25)

#Obtain the bottom 25% DEGs from our dataframe
dge_ai_bottom <- dge_ai |>
  slice_min(AI, prop = 0.25)
...
dge_cg_top <- dge_cg |>
  slice_max(CG, prop = 0.25)

dge_cg_bottom <- dge_cg |>
  slice_min(CG, prop = 0.25)
...
dge_dlpfc_top <- dge_dlpfc |>
  slice_max(DLPFC, prop = 0.25)

dge_dlpfc_bottom <- dge_dlpfc |>
  slice_min(DLPFC, prop = 0.25)
...
dge_na_top <- dge_na |>
  slice_max(NAC, prop = 0.25)

dge_na_bottom <- dge_na |>
  slice_min(NAC, prop = 0.25)
...
dge_ofc_top <- dge_ofc |>
  slice_max(OFC, prop = 0.25)

dge_ofc_bottom <- dge_ofc |>
  slice_min(OFC, prop = 0.25)
...
dge_sub_top <- dge_sub |>
  slice_max(SUB, prop = 0.25)

dge_sub_bottom <- dge_sub |>
  slice_min(SUB, prop = 0.25)
...
diffexp <- replace(diffexp, is.na(diffexp), 0) |>
  write_csv("results/4. Top 25% Heat Maps Post SVA/Males Meds/Human_DGE_L100025%_Males_Meds_Matrix.csv")

# Heatmap Generation ------------------------------------------------------

diff_expr_modified <- read_csv("results/4. Top 25% Heat Maps Post SVA/Males Meds/Human_DGE_L100025%_Males_Meds_Matrix.csv") |>
