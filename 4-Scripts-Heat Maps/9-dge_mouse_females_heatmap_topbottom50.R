# Extract top bottom 50% of genes from L1000 for each brain region and generate heat map (Mouse Females Comparison)
...
dge_pfc_symboled_top <- dge_pfc_symboled |>
  slice_max(PFC, prop = 0.5)

dge_pfc_symboled_bottom <- dge_pfc_symboled |>
  slice_min(PFC, prop = 0.5)
...
diffexp <- replace(diffexp, is.na(diffexp), 0) |>
  ...
  write_csv("results/5. Mouse Top 50% Heat Maps No SVA/Females/Mouse_DGE_L1000_50%_Females_Matrix.csv")

# Heatmap Generation ------------------------------------------------------

diff_expr_modified <- read_csv("results/5. Mouse Top 50% Heat Maps No SVA/Females/Mouse_DGE_L1000_50%_Females_Matrix.csv") |>
