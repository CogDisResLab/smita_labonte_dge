...
dge <- read.csv("Galaxy Output/Mouse NA/Mouse_NA_Overall_DGE.csv") %>%
...
count <- read.csv("Galaxy Count Matrix Output/Mouse NA/Mouse_NA_Overall_Counts.csv") %>%
  mutate(Geneid = as.character(Geneid))
...
colnames(lctmtx) <- paste0("Sample", 1:41)
