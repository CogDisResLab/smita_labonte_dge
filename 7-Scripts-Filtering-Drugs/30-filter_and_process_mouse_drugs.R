...
combinations <- expand_grid(
  ...
  b = c("Overall", "Males", "Females"), c = "drugfindr"
) |>
  mutate(
...
data_files <- combinations |>
  map(~ list.files("results/6. Mouse DrugFindR Output/",
...
filtered_05p_sinead <- data_files[str_detect(names(data_files), fixed("05p"))] |>
  ...
  ungroup() |>
  write_csv("results/cell-line-exploration/filtered_05p_sinead_mouse_drug_list_full.csv")

metadata <- get_ilincs_metadata(filtered_05p_sinead$signatureid) |>
...
final_filtered_05p_sinead <- filtered_05p_sinead |>
  ...
  unnest(pivoted) |>
  write_csv("results/cell-line-exploration/filtered_05p_sinead_mouse_drug_list_by_celllines.csv") |>
  nest(.by = c(region, comparison)) |>
  mutate(
    filename = str_glue("results/cell-line-exploration/filtered_05p_sinead_mouse_drug_list_{region}_{comparison}.csv")
  ) |>
