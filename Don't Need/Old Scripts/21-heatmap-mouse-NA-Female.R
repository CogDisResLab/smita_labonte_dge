...
dge <- read.csv("Galaxy Output/Mouse NA/Mouse_NA_Females_DGE.csv") %>%
...
count <- read.csv("Galaxy Count Matrix Output/Mouse NA/Mouse_NA_Female_Counts.csv") %>%
  mutate(Geneid = as.character(Geneid))
...
colnames(lctmtx) <- paste0("Sample", 1:21)
