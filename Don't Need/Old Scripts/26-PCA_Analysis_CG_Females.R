...
#Load CG metadata: Comparison MDD vs. CTRL for Females ------------------------
CG_metadata <- read_csv("data/GSE102556-CG-metadata.csv")
CG_count <-
  read_csv("Galaxy Count Matrix Mapped/CG/CG_Overall_Mapped_Final.csv") %>%
...
cg_count_female <- CG_count_filtered[, CG_metadata_truncated[,1] == "female"]

#Extract principal components
pca = prcomp(t(cg_count_female), scale = TRUE)
...
pca.data <- data.frame(Sample=rownames(pca$x),
                       ...
                       Death=CG_metadata$Cause_of_death[CG_metadata$gender == "female"])
pca.data
...
ggplot(data=pca.data, aes(x=X, y=Y, label=Sample, shape = Death, color = Diagnosis)) +
  ...
  theme_bw() +
  ggtitle("CG MDD vs CTRL (Females) PCA Graph") +
