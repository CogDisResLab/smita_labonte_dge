...
dge <- read.csv("Galaxy Output/DLPFC/DLPFC_Medication_DGE.csv") %>% mutate(GeneID = as.character(GeneID))
...
count <- read.csv("Galaxy Count Matrix Output/DLPFC/DLPFC_Male_MedCounts.csv") %>% mutate(Geneid = as.character(Geneid))
...
colnames(lctmtx) <- paste0("Sample", 1:9)
