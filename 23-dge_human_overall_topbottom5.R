...
#Obtain "overall" files for 6 human brain regions
files <- list.files("Galaxy Output/", "*.csv", full.names = TRUE, recursive = TRUE) |>
  keep(~ str_detect(.x, "Overall"))

#For each brain region, map all geneIDs to gene symbols and obtain logFC values for top and bottom 5% of DGEs
# Anterior Insula ---------------------------------------------------------

#Read the anterior insula overall CSV file and obtain the geneID and logFC values
...
#Obtain a dataframe containing only the gene symbol and its logFC value
...
dge_cg_symboled_top <- dge_cg_symboled |>
  slice_max(CG, prop = 0.05)

dge_cg_symboled_bottom <- dge_cg_symboled |>
  slice_min(CG, prop = 0.05)
...
dge_dlpfc_symboled_top <- dge_dlpfc_symboled |>
  slice_max(DLPFC, prop = 0.05)

dge_dlpfc_symboled_bottom <- dge_dlpfc_symboled |>
  slice_min(DLPFC, prop = 0.05)
...
dge_ofc_symboled_top <- dge_ofc_symboled |>
  slice_max(OFC, prop = 0.05)

dge_ofc_symboled_bottom <- dge_ofc_symboled |>
  slice_min(OFC, prop = 0.05)
...
diffexp <- replace(diffexp, is.na(diffexp), 0) |>
  ...
  write_csv("results/Human_DGE-L1000_Matrix_Overall_topbottom5.csv")


# Heatmap Generation ------------------------------------------------------


diff_expr_modified <- read_csv("results/Human_DGE-L1000_Matrix_Overall_topbottom5.csv") |>
