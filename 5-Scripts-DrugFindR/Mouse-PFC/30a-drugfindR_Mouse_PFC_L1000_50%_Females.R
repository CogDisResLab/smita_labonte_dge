...
# Comparison: PFC top/bottom 50% from L1000 Females
...
pfc_up_sig <- pfc_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse-PFC/50%fromL1000/Females/Mouse_PFC_L1000_50%_females_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
pfc_dn_sig <- pfc_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv("results/6. Mouse DrugFindR Output/Mouse-PFC/50%fromL1000/Females/Mouse_PFC_L1000_50%_females_bottom_genes_sig.csv")
...
x <- pfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse-PFC/50%fromL1000/Females/Mouse_PFC_L1000_50%_Females_drugfindr.xlsx")
