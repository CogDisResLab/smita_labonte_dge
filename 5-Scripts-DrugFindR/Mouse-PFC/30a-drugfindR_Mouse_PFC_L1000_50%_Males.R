...
# Comparison: PFC top/bottom 50% from L1000 Males

library(tidyverse)
library(drugfindR)

#Input DGE matrix with genes for our comparison of interest
dge <- read_csv("results/5. Mouse Top 50% Heat Maps No SVA/Males/Mouse_DGE_L1000_50%_Males_Matrix.csv") |>
...
pfc_up_sig <- pfc_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse-PFC/50%fromL1000/Males/Mouse_PFC_L1000_50%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
pfc_dn_sig <- pfc_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv("results/6. Mouse DrugFindR Output/Mouse-PFC/50%fromL1000/Males/Mouse_PFC_L1000_50%_males_bottom_genes_sig.csv")
...
x <- pfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse-PFC/50%fromL1000/Males/Mouse_PFC_L1000_50%_Males_drugfindr.xlsx")
