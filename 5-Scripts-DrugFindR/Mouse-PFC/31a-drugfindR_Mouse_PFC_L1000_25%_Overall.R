...
# Comparison: Mouse PFC top/bottom 25% from L1000 Overall

library(tidyverse)
library(drugfindR)

#Input DGE matrix with genes for our comparison of interest
dge <- read_csv("results/4. Mouse Top 25% Heat Maps No SVA/Overall/Mouse_DGE_L100025%_Matrix.csv") |>
...
pfc_up_sig <- pfc_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse-PFC/25%fromL1000/Overall/Mouse_PFC_L1000_25%_overall_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
pfc_dn_sig <- pfc_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. Mouse DrugFindR Output/Mouse-PFC/25%fromL1000/Overall/Mouse_PFC_L1000_25%_overall_bottom_genes_sig.csv")
...
x <- pfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse-PFC/25%fromL1000/Overall/Mouse_PFC_L1000_25%_Overall_drugfindr.xlsx")
