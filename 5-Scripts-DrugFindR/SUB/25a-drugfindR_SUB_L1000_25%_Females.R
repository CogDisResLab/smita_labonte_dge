...
# Comparison: SUB top/bottom 25% from L1000 Females
...
sub_up_sig <- sub_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/SUB/25%fromL1000/Females/SUB_L1000_25%_females_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
sub_dn_sig <- sub_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/SUB/25%fromL1000/Females/SUB_L1000_25%_females_bottom_genes_sig.csv")
...
x <- sub_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/SUB/25%fromL1000/Females/SUB_L1000_25%_Females_drugfindr.xlsx")
