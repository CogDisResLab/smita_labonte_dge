...
# Comparison: SUB top and bottom 10% from L1000 Females
...
sub_up_sig <- sub_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/SUB/10%fromL1000/Females/SUB_L1000_10%_females_top_genes_sig.csv")

#Extract bottom X% gene signature
sub_dn_sig <- sub_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/SUB/10%fromL1000/Females/SUB_L1000_10%_females_bottom_genes_sig.csv")
...
x <- sub_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/SUB/10%fromL1000/Females/SUB_L1000_10%_Females_drugfindr.xlsx")
