...
sub_up_sig <- sub_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/SUB/L1000/Males/SUB_males_top_genes_sig.csv")

#Extract bottom X% gene signature
sub_dn_sig <- sub_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/SUB/L1000/Males/SUB_males_bottom_genes_sig.csv")
...
x <- sub_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/SUB/L1000/Males/SUB_L1000_Males_drugfindr.xlsx")
