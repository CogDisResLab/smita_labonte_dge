...
# Comparison: SUB top/bottom 50% from L1000 Males
...
sub_up_sig <- sub_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/SUB/50%fromL1000/Males/SUB_L1000_50%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
sub_dn_sig <- sub_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/SUB/50%fromL1000/Males/SUB_L1000_50%_males_bottom_genes_sig.csv")
...
x <- sub_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/SUB/50%fromL1000/Males/SUB_L1000_50%_Males_drugfindr.xlsx")
