...
# Comparison: Mouse PFC top and bottom 5% from L1000 Males

library(tidyverse)
library(drugfindR)

#Input DGE matrix
dge <- read_csv("results/4. Mouse Top 5% Heat Maps No SVA/Males/Mouse_DGE_L10005%_Males_Matrix.csv")
...
pfc_up_sig <- pfc_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse PFC/5%fromL1000/Males/Mouse_L1000_5%_PFC_males_top_genes_sig.csv")

#Extract bottom X% gene signature
pfc_dn_sig <- pfc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. Mouse DrugFindR Output/Mouse PFC/5%fromL1000/Males/Mouse_L1000_5%_PFC_males_bottom_genes_sig.csv")
...
x <- pfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse PFC/5%fromL1000/Males/Mouse_PFC_L1000_5%_Males_drugfindr.xlsx")
