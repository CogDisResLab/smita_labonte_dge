...
# Comparison: Mouse PFC L1000 Males
...
pfc_up_sig <- pfc_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse PFC/L1000/Males/Mouse_PFC_males_top_genes_sig.csv")

#Extract bottom X% gene signature
pfc_dn_sig <- pfc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. Mouse DrugFindR Output/Mouse PFC/L1000/Males/Mouse_PFC_males_bottom_genes_sig.csv")
...
x <- pfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse PFC/L1000/Males/Mouse_PFC_L1000_Males_drugfindr.xlsx")
