...
# Comparison: Mouse PFC L1000 Overall
...
pfc_up_sig <- pfc_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse PFC/L1000/Overall/Mouse_PFC_overall_top_genes_sig.csv")

#Extract bottom X% gene signature
pfc_dn_sig <- pfc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. Mouse DrugFindR Output/Mouse PFC/L1000/Overall/Mouse_PFC_overall_bottom_genes_sig.csv")
...
x <- pfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse PFC/L1000/Overall/Mouse_PFC_L1000_Overall_drugfindr.xlsx")
