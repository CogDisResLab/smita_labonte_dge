...
cg_up_sig <- cg_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/CG/L1000/Males Meds/CG_males_meds_top_genes_sig.csv")

#Extract bottom X% gene signature
cg_dn_sig <- cg_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/CG/L1000/Males Meds/CG_males_meds_bottom_genes_sig.csv")
...
x <- cg_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/CG/L1000/Males Meds/CG_L1000_Males_Meds_drugfindr.xlsx")
