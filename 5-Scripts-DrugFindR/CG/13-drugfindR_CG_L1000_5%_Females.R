...
# Comparison: CG top and bottom 5% from L1000 Females
...
cg_up_sig <- cg_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/CG/5%fromL1000/Females/CG_L1000_5%_females_top_genes_sig.csv")

#Extract bottom X% gene signature
cg_dn_sig <- cg_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/CG/5%fromL1000/Females/CG_L1000_5%_females_bottom_genes_sig.csv")
...
x <- cg_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/CG/5%fromL1000/Females/CG_L1000_5%_Females_drugfindr.xlsx")
