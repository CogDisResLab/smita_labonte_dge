...
# Comparison: CG top/bottom 50% from L1000 Males Meds
...
cg_up_sig <- cg_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/CG/50%fromL1000/Males Meds/CG_L1000_50%_males_meds_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
cg_dn_sig <- cg_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/CG/50%fromL1000/Males Meds/CG_L1000_50%_males_meds_bottom_genes_sig.csv")
...
x <- cg_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/CG/50%fromL1000/Males Meds/CG_L1000_50%_Males_Meds_drugfindr.xlsx")
