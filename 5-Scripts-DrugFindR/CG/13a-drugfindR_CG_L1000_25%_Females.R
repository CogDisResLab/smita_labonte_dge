...
# Comparison: CG top/bottom 25% from L1000 Females
...
cg_up_sig <- cg_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/CG/25%fromL1000/Females/CG_L1000_25%_females_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
cg_dn_sig <- cg_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/CG/25%fromL1000/Females/CG_L1000_25%_females_bottom_genes_sig.csv")
...
x <- cg_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/CG/25%fromL1000/Females/CG_L1000_25%_Females_drugfindr.xlsx")
