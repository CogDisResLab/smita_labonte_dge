...
# Comparison: Mouse NAC top/bottom 25% from L1000 Females

library(tidyverse)
library(drugfindR)

#Input DGE matrix with genes for our comparison of interest
dge <- read_csv("results/4. Mouse Top 25% Heat Maps No SVA/Females/Mouse_DGE_L100025%_Females_Matrix.csv") |>
...
nac_up_sig <- nac_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse-NAC/25%fromL1000/Females/Mouse_NAC_L1000_25%_females_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
nac_dn_sig <- nac_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. Mouse DrugFindR Output/Mouse-NAC/25%fromL1000/Females/Mouse_NAC_L1000_25%_females_bottom_genes_sig.csv")
...
x <- nac_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse-NAC/25%fromL1000/Females/Mouse_NAC_L1000_25%_Females_drugfindr.xlsx")
