...
# Comparison: NAC top/bottom 50% from L1000 Males
...
nac_up_sig <- nac_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse-NAC/50%fromL1000/Males/Mouse_NAC_L1000_50%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
nac_dn_sig <- nac_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv("results/6. Mouse DrugFindR Output/Mouse-NAC/50%fromL1000/Males/Mouse_NAC_L1000_50%_males_bottom_genes_sig.csv")
...
x <- nac_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse-NAC/50%fromL1000/Males/Mouse_NAC_L1000_50%_Males_drugfindr.xlsx")
