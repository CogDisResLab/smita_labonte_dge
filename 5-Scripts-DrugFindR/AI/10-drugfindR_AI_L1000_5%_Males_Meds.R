...
# Comparison: AI top/bottom 5% from L1000 Males Meds
...
ai_up_sig <- ai_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/AI/5%fromL1000/Males Meds/AI_L1000_5%_males_meds_top_genes_sig.csv")

#Extract bottom X% gene signature
ai_dn_sig <- ai_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/AI/5%fromL1000/Males Meds/AI_L1000_5%_males_meds_bottom_genes_sig.csv")
...
x <- ai_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/AI/5%fromL1000/Males Meds/AI_L1000_5%_Males_Meds_drugfindr.xlsx")
