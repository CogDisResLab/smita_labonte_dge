...
# Comparison: AI top/bottom 5% from L1000 Males
...
ai_up_sig <- ai_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/AI/25%fromL1000/Males/AI_L1000_25%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
ai_dn_sig <- ai_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/AI/25%fromL1000/Males/AI_L1000_25%_males_bottom_genes_sig.csv")
...
x <- ai_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/AI/25%fromL1000/Males/AI_L1000_25%_Males_drugfindr.xlsx")
