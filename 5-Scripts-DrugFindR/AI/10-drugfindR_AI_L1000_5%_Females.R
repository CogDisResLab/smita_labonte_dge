...
# Comparison: AI top/bottom 5% from L1000 Females
...
ai_dge_sig <- dge |>
  prepare_signature(
    gene_column = "Name_GeneSymbol", logfc_column = "AI",
    pval_column = "AI_Pval"
  )
...
ai_up_sig <- ai_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/AI/5%fromL1000/Females/AI_L1000_5%_females_top_genes_sig.csv")

# Create a file listing bottom X% gene signature
ai_dn_sig <- ai_dge_sig |>
  filter_signature("down", threshold = 0) |>
  write_csv("results/6. DrugFindR Output/AI/5%fromL1000/Females/AI_L1000_5%_females_bottom_genes_sig.csv")
...
x <- ai_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/AI/5%fromL1000/Females/AI_L1000_5%_Females_drugfindr.xlsx")
