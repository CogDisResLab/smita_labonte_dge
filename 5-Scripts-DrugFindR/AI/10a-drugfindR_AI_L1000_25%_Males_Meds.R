...
# Comparison: AI top/bottom 25% from L1000 Males Meds

library(tidyverse)
library(drugfindR)

#Input DGE matrix with genes for our comparison of interest
dge <- read_csv("results/4. Top 25% Heat Maps Post SVA/Males Meds/Human_DGE_L100025%_Males_Meds_Matrix.csv") |>
...
ai_up_sig <- ai_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/AI/25%fromL1000/Males Meds/AI_L1000_25%_males_meds_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
ai_dn_sig <- ai_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/AI/25%fromL1000/Males Meds/AI_L1000_25%_males_meds_bottom_genes_sig.csv")
...
x <- ai_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/AI/25%fromL1000/Males Meds/AI_L1000_25%_Males_Meds_drugfindr.xlsx")
