...
# Comparison: AI top/bottom 50% from L1000 Males

library(tidyverse)
library(drugfindR)

#Input DGE matrix with genes for our comparison of interest
dge <- read_csv("results/5. Top 50% Heat Maps Post SVA/Males/Human_DGE_L1000_50%_Males_Matrix.csv") |>
...
ai_up_sig <- ai_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/AI/50%fromL1000/Males/AI_L1000_50%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
ai_dn_sig <- ai_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/AI/50%fromL1000/Males/AI_L1000_50%_males_bottom_genes_sig.csv")
...
x <- ai_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/AI/50%fromL1000/Males/AI_L1000_50%_Males_drugfindr.xlsx")
