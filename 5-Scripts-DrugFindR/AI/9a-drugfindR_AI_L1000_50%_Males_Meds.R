...
# Comparison: AI top/bottom 50% from L1000 Males Meds

library(tidyverse)
library(drugfindR)

#Input DGE matrix with genes for our comparison of interest
dge <- read_csv("results/5. Top 50% Heat Maps Post SVA/Males Meds/Human_DGE_L1000_50%_Males_Meds_Matrix.csv") |>
  select(Name_GeneSymbol, starts_with("AI")) %>%
  filter(AI != 0)

#Extract gene signature (gene name and LFC values)
ai_dge_sig <- dge |>
  prepareSignature(geneColumn = "Name_GeneSymbol", logfcColumn = "AI",
                    pvalColumn = "AI_Pval")

#Create a file listing top X% gene signature
ai_up_sig <- ai_dge_sig |>
  filterSignature("up", threshold = 0) |>
  write_csv("results/6. DrugFindR Output/AI/50%fromL1000/Males Meds/AI_L1000_50%_males_meds_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
ai_dn_sig <- ai_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/AI/50%fromL1000/Males Meds/AI_L1000_50%_males_meds_bottom_genes_sig.csv")
...
x <- ai_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/AI/50%fromL1000/Males Meds/AI_L1000_50%_Males_Meds_drugfindr.xlsx")
