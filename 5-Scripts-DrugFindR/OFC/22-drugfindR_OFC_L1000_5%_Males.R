...
# Comparison: OFC top and bottom 5% from L1000 Males
...
ofc_up_sig <- ofc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/OFC/5%fromL1000/Males/OFC_L1000_5%_males_top_genes_sig.csv")

#Extract bottom X% gene signature
ofc_dn_sig <- ofc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/OFC/5%fromL1000/Males/OFC_L1000_5%_males_bottom_genes_sig.csv")
...
x <- ofc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/OFC/5%fromL1000/Males/OFC_L1000_5%_Males_drugfindr.xlsx")
