...
# Comparison: OFC top and bottom 10% from L1000 Females
...
ofc_up_sig <- ofc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/OFC/10%fromL1000/Females/OFC_L1000_10%_females_top_genes_sig.csv")

#Extract bottom X% gene signature
ofc_dn_sig <- ofc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/OFC/10%fromL1000/Females/OFC_L1000_10%_females_bottom_genes_sig.csv")
...
x <- ofc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/OFC/10%fromL1000/Females/OFC_L1000_10%_Females_drugfindr.xlsx")
