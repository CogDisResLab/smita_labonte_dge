...
# Comparison: OFC top/bottom 25% from L1000 Males Meds
...
ofc_up_sig <- ofc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/OFC/25%fromL1000/Males Meds/OFC_L1000_25%_males_meds_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
ofc_dn_sig <- ofc_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/OFC/25%fromL1000/Males Meds/OFC_L1000_25%_males_meds_bottom_genes_sig.csv")
...
x <- ofc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/OFC/25%fromL1000/Males Meds/OFC_L1000_25%_Males_Meds_drugfindr.xlsx")
