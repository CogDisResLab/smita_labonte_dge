...
# Comparison: OFC top/bottom 50% from L1000 Males
...
ofc_up_sig <- ofc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/OFC/50%fromL1000/Males/OFC_L1000_50%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
ofc_dn_sig <- ofc_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/OFC/50%fromL1000/Males/OFC_L1000_50%_males_bottom_genes_sig.csv")
...
x <- ofc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/OFC/50%fromL1000/Males/OFC_L1000_50%_Males_drugfindr.xlsx")
