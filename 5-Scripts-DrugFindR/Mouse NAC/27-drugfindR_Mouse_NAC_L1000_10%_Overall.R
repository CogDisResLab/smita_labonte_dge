...
# Comparison: Mouse top and bottom 10% from NAC L1000 Overall

library(tidyverse)
library(drugfindR)

#Input DGE matrix
dge <- read_csv("results/5. Mouse Top 10% Heat Maps No SVA/Overall/Mouse_DGE_L1000_10%_Matrix.csv")
...
nac_up_sig <- nac_dge_sig |>
  ...
  write_csv("results/6. Mouse DrugFindR Output/Mouse NAC/10%fromL1000/Overall/Mouse_NAC_L1000_10%_overall_top_genes_sig.csv")

#Extract bottom X% gene signature
nac_dn_sig <- nac_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. Mouse DrugFindR Output/Mouse NAC/10%fromL1000/Overall/Mouse_NAC_L1000_10%_overall_bottom_genes_sig.csv")
...
x <- nac_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. Mouse DrugFindR Output/Mouse NAC/10%fromL1000/Overall/Mouse_NAC_L1000_10%_Overall_drugfindr.xlsx")
