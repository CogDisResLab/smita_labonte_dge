...
# Comparison: DLPFC top and bottom 10% from L1000 Males Meds
...
dlpfc_up_sig <- dlpfc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/DLPFC/10%fromL1000/Males Meds/DLPFC_L1000_10%_males_meds_top_genes_sig.csv")

#Extract bottom X% gene signature
dlpfc_dn_sig <- dlpfc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/DLPFC/10%fromL1000/Males Meds/DLPFC_L1000_10%_males_meds_bottom_genes_sig.csv")
...
x <- dlpfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/DLPFC/10%fromL1000/Males Meds/DLPFC_L1000_10%_Males_Meds_drugfindr.xlsx")
