...
# Comparison: DLPFC top and bottom 10% from L1000 Overall

library(tidyverse)
library(drugfindR)

#Input DGE matrix
dge <- read_csv("results/5. Top 10% Heat Maps Post SVA/Overall/Human_DGE_L1000_10%_Matrix.csv")
...
dlpfc_up_sig <- dlpfc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/DLPFC/10%fromL1000/Overall/DLPFC_L1000_10%_overall_top_genes_sig.csv")

#Extract bottom X% gene signature
dlpfc_dn_sig <- dlpfc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/DLPFC/10%fromL1000/Overall/DLPFC_L1000_10%_overall_bottom_genes_sig.csv")
...
x <- dlpfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/DLPFC/10%fromL1000/Overall/DLPFC_L1000_10%_Overall_drugfindr.xlsx")
