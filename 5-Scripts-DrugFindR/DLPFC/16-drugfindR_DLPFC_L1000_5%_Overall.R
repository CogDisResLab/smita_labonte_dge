...
# Comparison: DLPFC top and bottom 15% from L1000 Overall
...
dlpfc_up_sig <- dlpfc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/DLPFC/5%fromL1000/Overall/DLPFC_L1000_5%_overall_top_genes_sig.csv")

#Extract bottom X% gene signature
dlpfc_dn_sig <- dlpfc_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/DLPFC/5%fromL1000/Overall/DLPFC_L1000_5%_overall_bottom_genes_sig.csv")
...
x <- dlpfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/DLPFC/5%fromL1000/Overall/DLPFC_L1000_5%_Overall_drugfindr.xlsx")
