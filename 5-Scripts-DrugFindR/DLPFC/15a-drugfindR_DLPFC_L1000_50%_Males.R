...
# Comparison: DLPFC top/bottom 50% from L1000 Males
...
dlpfc_up_sig <- dlpfc_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/DLPFC/50%fromL1000/Males/DLPFC_L1000_50%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
dlpfc_dn_sig <- dlpfc_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/DLPFC/50%fromL1000/Males/DLPFC_L1000_50%_males_bottom_genes_sig.csv")
...
x <- dlpfc_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/DLPFC/50%fromL1000/Males/DLPFC_L1000_50%_Males_drugfindr.xlsx")
