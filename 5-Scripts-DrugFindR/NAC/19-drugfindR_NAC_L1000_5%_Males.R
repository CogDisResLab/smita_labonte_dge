...
# Comparison: NAC top and bottom 5% from L1000 Males
...
nac_up_sig <- nac_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/NAC/5%fromL1000/Males/NAC_L1000_5%_males_top_genes_sig.csv")

#Extract bottom X% gene signature
nac_dn_sig <- nac_dge_sig |>
  filter_signature("down", prop = 0.05) |>
  write_csv ("results/6. DrugFindR Output/NAC/5%fromL1000/Males/NAC_L1000_5%_males_bottom_genes_sig.csv")
...
x <- nac_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/NAC/5%fromL1000/Males/NAC_L1000_5%_Males_drugfindr.xlsx")
