...
# Comparison: NAC top/bottom 25% from L1000 Males
...
nac_up_sig <- nac_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/NAC/25%fromL1000/Males/NAC_L1000_25%_males_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
nac_dn_sig <- nac_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/NAC/25%fromL1000/Males/NAC_L1000_25%_males_bottom_genes_sig.csv")
...
x <- nac_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/NAC/25%fromL1000/Males/NAC_L1000_25%_Males_drugfindr.xlsx")
