...
# Comparison: NAC top/bottom 50% from L1000 Males Meds
...
dge <- read_csv("results/5. Top 50% Heat Maps Post SVA/Males Meds/Human_DGE_L1000_50%_Males_Meds_Matrix.csv") |>
  select(Name_GeneSymbol, starts_with("NA")) |>
  filter(NAC != 0)
...
nac_up_sig <- nac_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/NAC/50%fromL1000/Males Meds/NAC_L1000_50%_males_meds_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
nac_dn_sig <- nac_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/NAC/50%fromL1000/Males Meds/NAC_L1000_50%_males_meds_bottom_genes_sig.csv")
...
x <- nac_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/NAC/50%fromL1000/Males Meds/NAC_L1000_50%_Males_Meds_drugfindr.xlsx")
