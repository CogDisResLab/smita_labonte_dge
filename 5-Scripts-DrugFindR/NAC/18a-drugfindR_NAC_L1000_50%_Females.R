...
nac_up_sig <- nac_dge_sig |>
  ...
  write_csv("results/6. DrugFindR Output/NAC/50%fromL1000/Females/NAC_L1000_50%_females_top_genes_sig.csv")

#Create a file listing bottom X% gene signature
nac_dn_sig <- nac_dge_sig |>
  filterSignature("down", threshold = 0) |>
  write_csv ("results/6. DrugFindR Output/NAC/50%fromL1000/Females/NAC_L1000_50%_females_bottom_genes_sig.csv")
...
x <- nac_concordants_all %>%
  ...
  deframe() %>%
  writexl::write_xlsx("results/6. DrugFindR Output/NAC/50%fromL1000/Females/NAC_L1000_50%_Females_drugfindr.xlsx")
