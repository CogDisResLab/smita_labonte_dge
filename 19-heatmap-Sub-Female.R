...
dge <- read.csv("Galaxy Output/Sub/Sub_Female_DGE.csv") %>% mutate(GeneID = as.character(GeneID))
...
count <- read.csv("Galaxy Count Matrix Output/Sub/Sub_Female_Counts.csv") %>% mutate(Geneid = as.character(Geneid))
...
colnames(lctmtx) <- paste0("Sample", 1:19)
